...
const PeerRating2 = () => {
  ...
  // Get remainingTeammates, teammate, and teamName from location state
  ...
  useEffect(() => {
    ...
    console.log("remainingTeammates:", remainingTeammates); // Check remaining teammates
    console.log("teamName:", teamName); // Check teamName
  }, [teamId, studentId, remainingTeammates, teamName]);

  const submitRating = async () => {
    if (!conceptualContribution || conceptualContribution < 1 || conceptualContribution > 5) {
      setError('Please select a valid conceptual contribution rating (1-5).');
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { rateeId: studentId, conceptualContribution, comment },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      console.log('Response:', response);

      // Navigate back to Conceptual Contribution with updated remainingTeammates
      navigate('/conceptual_contribution', {
        state: {
          updatedTeammates: remainingTeammates.filter((t) => t._id !== studentId),
          teamId,
          teamName, // Pass teamName back
        },
      });
    } catch (error) {
        ...
        <header className="bg-white shadow">
          <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <h1 className="text-3xl font-bold tracking-tight text-gray-900">
              Team Conceptual Contribution Evaluation
            </h1>
          </div>
        </header>
            ...
            <div className="w-full flex flex-col">
              <div className="mt-6 w-1/2">
                <div className="mb-4">
                  ...
                  <br />
                  <label htmlFor="conceptualContribution" className="block text-lg font-medium text-gray-900">
                    Conceptual Contribution Rating (1-5):
                  </label>
                  <select
                    id="conceptualContribution"
                    value={conceptualContribution}
                    onChange={(e) => setConceptualContribution(e.target.value)}
