...
const Cooperation = () => {
  ...
  const handleRate = (teammate) => {
    if (!teamId) {
      console.error("Team ID is undefined, cannot navigate to PeerRating.");
      return;
    }
      ...
      <main>
        <div className="py-6">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <h2 className="mt-2 text-lg">Rate your teammates:</h2>
            <ul className="mt-4">
              {selectedTeammates.map(teammate => (
                <li key={teammate._id} className="mt-2">
                  <span>{teammate.firstName} {teammate.lastName}</span>
                  <button onClick={() => handleRate(teammate)} className="ml-4 rounded bg-blue-500 px-4 py-2 text-white">Rate</button>
                </li>
              ))}
            </ul>
            <button onClick={handleGoBack} className="mt-6 rounded bg-gray-500 px-4 py-2 text-white">Go Back</button>
          </div>
        </div>
      </main>
    </div>
