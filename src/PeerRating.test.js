...
describe('PeerRating Component', () => {
  ...
  beforeEach(() => {
    ...
    mockUseLocation.mockReturnValue({
      state: {
        ...
        remainingTeammates: [{ _id: '1', firstName: 'John', lastName: 'Doe' }],
        teamName: 'Team Alpha',
      },
    });
  ...
  it('should display error if no cooperation rating is provided', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Please select a valid cooperation rating (1-5).')).toBeInTheDocument());
  });
  ...
  it('should display error if no comment is provided', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Cooperation Rating'), { target: { value: '5' } });
    fireEvent.click(screen.getByText('Submit Rating'));

    await waitFor(() => expect(screen.getByText('Please enter a comment.')).toBeInTheDocument());
  });
  ...
  it('should submit the rating and navigate back to Cooperation page', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Cooperation Rating'), { target: { value: '5' } });
    fireEvent.change(screen.getByLabelText('Comment'), { target: { value: 'Great teammate!' } });
    ...
    await waitFor(() => {
      ...
      expect(mockNavigate).toHaveBeenCalledWith('/cooperation', expect.objectContaining({
